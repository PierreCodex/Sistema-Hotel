...
function actualizarTabla(lista) {
    ...
    lista.forEach(item => {
        // Tipo de documento (usando campos unificados comp_)
        const tipoBadge = item.comp_tipo == '03' 
            ? '<span class="badge bg-info">Boleta</span>'
            : '<span class="badge bg-success">Factura</span>';
        
        // Estado
        let estadoBadge = '';
        switch(item.comp_estado) {
            case 'ACEPTADA':
            ...
            case 'ANULADA':
                ...
                break;
            default:
                estadoBadge = '<span class="badge bg-light text-dark">' + item.comp_estado + '</span>';
        }
        
        // Botones de acción (pasar origen para saber si es boleta o factura)
        const origen = item.origen_tabla || 'boleta';
        const acciones = `
            <div class="btn-group btn-group-sm">
                <button type="button" class="btn btn-soft-primary" onclick="verDetalle(${item.comp_id}, '${origen}')" title="Ver detalle">
                    <i class="ri-eye-line"></i>
                </button>
                <button type="button" class="btn btn-soft-danger" onclick="descargarPDF(${item.comp_id}, '${origen}')" title="Descargar PDF">
                    <i class="ri-file-pdf-line"></i>
                </button>
                <button type="button" class="btn btn-soft-success" onclick="descargarXML(${item.comp_id}, '${origen}')" title="Descargar XML">
                    <i class="ri-code-s-slash-line"></i>
                </button>
            </div>
        `;
        
        totalSubtotal += parseFloat(item.comp_subtotal) || 0;
        totalIgv += parseFloat(item.comp_igv) || 0;
        totalGeneral += parseFloat(item.comp_total) || 0;
        
        dataTable.row.add([
            tipoBadge,
            `<strong>${item.comp_serie}-${item.comp_correlativo}</strong>`,
            formatearFechaHora(item.comp_fecha_emision),
            item.comp_cliente_razon_social || 'Cliente General',
            item.comp_cliente_num_doc || '-',
            'S/ ' + formatearMonto(item.comp_subtotal),
            'S/ ' + formatearMonto(item.comp_igv),
            '<strong>S/ ' + formatearMonto(item.comp_total) + '</strong>',
            estadoBadge,
            acciones
        ]);
    });
...
function verDetalle(comp_id, origen = 'boleta') {
    const formData = new FormData();
...
function mostrarModalDetalle(comprobante, detalles) {
    const tipoDoc = comprobante.comp_tipo == '03' ? 'BOLETA DE VENTA ELECTRÓNICA' : 'FACTURA ELECTRÓNICA';
    const estadoClass = comprobante.comp_estado == 'ACEPTADA' ? 'success' : 
                        comprobante.comp_estado == 'RECHAZADA' ? 'danger' : 'warning';
    ...
    detalles.forEach((det, index) => {
        detallesHTML += `
            <tr>
                <td>${index + 1}</td>
                <td>${det.det_codigo}</td>
                <td>${det.det_descripcion}</td>
                <td class="text-center">${det.det_cantidad}</td>
                <td class="text-end">S/ ${formatearMonto(det.det_precio_unitario)}</td>
                <td class="text-end">S/ ${formatearMonto(det.det_total)}</td>
            </tr>
        `;
    });
    ...
    const html = `
        <div class="row mb-4">
            <div class="col-md-6">
                ...
                <p class="mb-0"><strong>${comprobante.comp_serie}-${comprobante.comp_correlativo}</strong></p>
                <small class="text-muted">Emitido: ${formatearFechaHora(comprobante.comp_fecha_emision)}</small>
            </div>
            <div class="col-md-6 text-md-end">
                <span class="badge bg-${estadoClass} fs-6">${comprobante.comp_estado}</span>
                <p class="mb-0 mt-2"><small class="text-muted">Método de pago:</small></p>
                <p class="mb-0"><strong>${comprobante.comp_metodo_pago || 'EFECTIVO'}</strong></p>
            </div>
        </div>
                    ...
                    <div class="card-body py-2">
                        <div class="row">
                            <div class="col-md-6">
                                ...
                                <p class="mb-0 fw-semibold">${comprobante.comp_cliente_razon_social || 'CLIENTE GENERAL'}</p>
                            </div>
                            <div class="col-md-3">
                                <small class="text-muted">Documento:</small>
                                <p class="mb-0 fw-semibold">${comprobante.comp_cliente_num_doc || '-'}</p>
                            </div>
                            <div class="col-md-3">
                                <small class="text-muted">Dirección:</small>
                                <p class="mb-0">${comprobante.comp_cliente_direccion || '-'}</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <table class="table table-bordered table-sm">
                ...
                <tfoot>
                    <tr>
                        ...
                        <td class="text-end">S/ ${formatearMonto(comprobante.comp_subtotal)}</td>
                    </tr>
                    <tr>
                        <td colspan="5" class="text-end"><strong>IGV (18%):</strong></td>
                        <td class="text-end">S/ ${formatearMonto(comprobante.comp_igv)}</td>
                    </tr>
                    <tr class="table-primary">
                        <td colspan="5" class="text-end"><strong>TOTAL:</strong></td>
                        <td class="text-end"><strong>S/ ${formatearMonto(comprobante.comp_total)}</strong></td>
                    </tr>
                </tfoot>
            </table>
        </div>
        
        ${comprobante.comp_descripcion_cdr ? `
        <div class="alert alert-info mt-3">
            <i class="ri-information-line me-2"></i>
            <strong>Respuesta SUNAT:</strong> ${comprobante.comp_descripcion_cdr}
        </div>
        ` : ''}
    `;
    ...
    document.getElementById('modalDetalleLabel').innerHTML = `
        <i class="ri-file-list-3-line me-2"></i>${tipoDoc} ${comprobante.comp_serie}-${comprobante.comp_correlativo}
    `;
...
// Descargar PDF
function descargarPDF(comp_id, origen = 'boleta') {
    window.open(`../../controller/comprobante.php?operacion=descargar_pdf&comp_id=${comp_id}&origen=${origen}`, '_blank');
}

// Descargar XML
function descargarXML(comp_id, origen = 'boleta') {
    window.location.href = `../../controller/comprobante.php?operacion=descargar_xml&comp_id=${comp_id}&origen=${origen}`;
}
